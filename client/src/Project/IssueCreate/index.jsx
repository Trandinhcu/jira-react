...
const ProjectIssueCreate = ({ project, fetchProject, onCreate, modalClose }) => {
  ...
  return (
    <Form
      ...
      onSubmit={async (values, form) => {
        try {
          ...
          await fetchProject();
          toast.success('Issue đã được tạo thành công.');
          onCreate();
        } catch (error) {
    ...
    >
      <FormElement>
        <FormHeading>Tạo issue</FormHeading>
        <Form.Field.Select
          name="type"
          label="Loại vấn đề"
          options={typeOptions}
        ...
        />
        <Divider />
        <Form.Field.Input name="title" label="Mô tả ngắn" />
        <Form.Field.TextEditor name="description" label="Mô tả" />
        <Form.Field.Select
          name="reporterId"
          label="Người báo cáo"
