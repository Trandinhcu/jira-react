...
const Projects = () => {
  ...
  const columnsConfig = [
    ...
    {
      title: (
        <div style={{ fontSize: '14px', fontWeight: '700', color: '#777' }}>Thời gian bắt đầu</div>
      ),
      dataIndex: 'timeStart',
      key: 'timeStart',
      align: 'center',
      render: timeStart => moment(timeStart).format('DD/MM/YYYY'),
    },
    {
      title: (
        <div style={{ fontSize: '14px', fontWeight: '700', color: '#777' }}>Thời gian kết thúc</div>
      ),
      dataIndex: 'timeEnd',
      key: 'timeEnd',
      align: 'center',
      render: timeEnd => moment(timeEnd).format('DD/MM/YYYY'),
    },
    {
  ...
  const onCreateProject = async value => {
    try {
      await projectActions.createProject({
        ...
        timeStart: moment(value.timeStart),
        timeEnd: moment(value.timeEnd),
      });
      handleGetAllProject();
    } catch ({ response }) {
      const { data } = response;
  ...
  return (
    <ProjectPage>
      ...
      <ProjectsContainer>
        ...
        >
          <h1>DỰ ÁN</h1>
          <Input placeholder="Tìm kiếm" name='searchTerm' value={filter.searchTerm} onChange={handleFilter} />
          <Button
            style={{
              display: 'flex',
              alignItems: 'center',
