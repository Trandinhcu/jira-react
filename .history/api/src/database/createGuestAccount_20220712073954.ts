import { Comment, Issue, Project, Role, User } from 'entities';
...
const seedUsers = (roles: Role[]): Promise<User[]> => {
  console.log(roles.find(item => item.name == RoleName.ADMIN)?.id, 'role ADMIN');

  const users = [
    createEntity(User, {
      email: 'admin@gmail.com',
      password: crypto.createHmac('SHA256', process.env.JWT_SECRET,).update("123456").digest('base64'),
      roleId: roles.find(item => item.name == RoleName.ADMIN)?.id,
      name: 'Pickle Rick',
    ...
    createEntity(User, {
      email: 'pm@gmail.com',
      password: crypto.createHmac('SHA256', process.env.JWT_SECRET,).update("123456").digest('base64'),
      roleId: roles.find(item => item.name == RoleName.PM)?.id,
      name: 'Baby Yoda',
    ...
    createEntity(User, {
      email: 'ba@gmail.com',
           password: crypto.createHmac('SHA256', process.env.JWT_SECRET,).update("123456").digest('base64'),
      roleId: roles.find(item => item.name == RoleName.BA)?.id,
      name: 'Baby Yoda',
      avatarUrl: 'https://i.ibb.co/6n0hLML/baby-yoda.jpg',
    }),
    createEntity(User, {
      email: 'dev@gmail.com',
           password: crypto.createHmac('SHA256', process.env.JWT_SECRET,).update("123456").digest('base64'),
      name: 'dev',
      roleId: roles.find(item => item.name == RoleName.DEV)?.id,
    ...
    createEntity(User, {
      email: 'tester@gmail.com',
           password: crypto.createHmac('SHA256', process.env.JWT_SECRET,).update("123456").digest('base64'),
      name: 'tester',
      roleId: roles.find(item => item.name == RoleName.TESTER)?.id,
      avatarUrl: 'https://i.ibb.co/6RJ5hq6/gaben.jpg',
    }),
  ];
