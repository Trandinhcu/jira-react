...
import {Role, Project, User, Issue, Comment } from 'entities';
import { EntityNotFoundError, BadUserInputError } from 'errors';
import { generateErrors } from 'utils/validation';

type EntityConstructor = typeof Project | typeof User | typeof Issue | typeof Comment | typeof Role;
type EntityInstance = Project | User | Issue | Comment | Role;
...
export const validateAndSaveEntity = async <T extends EntityInstance>(instance: T): Promise<T> => {
  ...
  if(!Constructor)return instance.save() as Promise<T>;;
